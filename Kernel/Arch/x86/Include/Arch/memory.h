...
struct PAGE_TABLE
{
    ...
    uint32 GetFlags(uint32 index) { return values[index] & ~PTE_FRAME; }
    ...
    void *GetAddr(uint32 index) { return (void *)(values[index] & PTE_FRAME); }
...
struct PAGE_DIR
{
    ...
    uint32 GetFlags(uint32 index) { return values[index] & ~PDE_FRAME; }
    ...
    PAGE_TABLE *GetTable(uint32 index) { return (PAGE_TABLE *)(values[index] & PDE_FRAME); }
} __attribute__((packed));
