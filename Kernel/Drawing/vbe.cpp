...
VBE_MODE_INFO VBE::GetMode()
{
	return mode;
}

void VBE::Draw(uint32* framebuffer)
{
	memcpy(mem_base, framebuffer, mem_size);
}

void VBE::SetPixel(int x, int y, uint32 col)
{
	if (x >= mode.width || x < 0)
		return;
...
void VBE::DrawText(int x, int y, char* c, uint32 fg, uint32 bg)
{
