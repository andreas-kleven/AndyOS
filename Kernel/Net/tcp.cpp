...
TcpSession* TCP::sessions[MAX_SESSIONS];

void TCP::Send(NetInterface* intf, NetPacket* pkt)
{
	//checksum
	IPv4::Send(intf, pkt);
}

NetPacket* TCP::CreatePacket(NetInterface* intf, IPv4Address dst, uint16 src_port, uint16 dst_port, uint8 flags, uint32 seq, uint32 ack, uint8* data, uint32 data_length)
{
...
bool TCP::Decode(TCP_Packet* tp, NetPacket* pkt)
{
