...
namespace ProcessManager
{
    ...
    THREAD *InterruptSyscalls(THREAD *thread)
    {
        ...
        if (dispatcher_entry)
        {
            ...
            debug_print("Handler thread %d %d\n", handler_thread->id, dispatcher_entry->context.syscall);

            thread->signal_threads.Push(handler_thread);

            handler_thread->interrupted = true;

            if (handler_thread->sleep_until || (handler_thread->event && handler_thread->event_interruptible))
            {
                debug_print("Interrupting handler %d %d %d %d\n", handler_thread->id, handler_thread->state, handler_thread->event, handler_thread->sleep_until);
