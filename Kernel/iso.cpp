...
{
	char* buf;
	if (!ATA::Read(ATA_BUS_PRIMARY, ATA_DRIVE_MASTER, 0x10, buf, 0x1000))
	...
	if (!ATA::Read(ATA_BUS_PRIMARY, ATA_DRIVE_MASTER, table->locationLBA, buf, ISO_SECTOR_SIZE * 2))
		return STATUS_FAILED;
...
{
	return ATA::Read(ATA_BUS_PRIMARY, ATA_DRIVE_MASTER, file->locationLBA_LSB, buffer, file->filesize_LSB);
}
