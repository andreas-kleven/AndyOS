...
NetInterface* KNet::intf;

MacAddress KNet::GatewayMAC;
IPv4Address KNet::GatewayIPv4;

MacAddress KNet::BroadcastMAC;
IPv4Address KNet::BroadcastIPv4;

MacAddress KNet::NullMAC;
IPv4Address KNet::NullIPv4;
...
uint16 KNet::Checksum(void* vdata, int length)
{
	uint32 acc = 0xFFFF;
...
uint16 KNet::ChecksumDouble(void* d0, int l0, void* d1, int l1)
{
	uint32 acc = 0xFFFF;
...
void KNet::PrintIP(char* str, IPv4Address ip)
{
	uint8* addr = (uint8*)&ip;
	Debug::Print("%s%i.%i.%i.%i\n", str, addr[0], addr[1], addr[2], addr[3]);
}

void KNet::PrintMac(char* str, MacAddress mac)
{
