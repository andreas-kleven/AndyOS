...
Vector3 transform(Rigidbody &body, Vector3 point)
{
    ...
    point = Vector3(point.x * transform.scale.x, point.y * transform.scale.y,
...
Vector3 transform2(Rigidbody &body, Vector3 point)
{
    ...
    // pos = Vector3(pos.x / transform.scale.x, pos.y / transform.scale.y, pos.z /
    // transform.scale.z);
    return -transform.rotation * pos;
}

bool Collision::TestIntersection(Rigidbody &o1, Rigidbody &o2, Vector3 *mtv, Manifold *&col,
                                 int &count)
{
...
Manifold *Collision::CollisionPoint(Rigidbody &obj1, Rigidbody &obj2, int &count)
{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (isInside(CP, obj1)) {
            ...
            if (collisionNormal == Vector3(0, 0, 0)) {
                collisionNormal =
                    (obj2.parent->GetWorldPosition() - obj1.parent->GetWorldPosition())
                        .Normalized();
            }
        ...
        if (isInside(CP, obj2)) {
            ...
            if (collisionNormal == Vector3(0, 0, 0)) {
                collisionNormal =
                    (obj1.parent->GetWorldPosition() - obj2.parent->GetWorldPosition())
                        .Normalized();
            }
    ...
    for (int i = 0; i < 12; i++) {
        ...
        for (int j = 0; j < 12; j++) {
            ...
            closest_Point(edgePoint1a, edgePoint1b, edgePoint2a, edgePoint2b, colisionPoint1,
                          colisionPoint2);
...
bool Collision::isInside(Vector3 ip, Rigidbody obj1)
{
    ...
    if (localP.x < -obj1.collider->size.x * scale.x || localP.x > obj1.collider->size.x * scale.x)
        return 0;
    if (localP.y < -obj1.collider->size.y * scale.y || localP.y > obj1.collider->size.y * scale.y)
        return 0;
    if (localP.z < -obj1.collider->size.z * scale.z || localP.z > obj1.collider->size.z * scale.z)
        return 0;
    return 1;
}

// Method finds the closest point between 2 lines, used for edge to edge collisions
void Collision::closest_Point(Vector3 p1, Vector3 q1, Vector3 p2, Vector3 q2, Vector3 &c1,
                              Vector3 &c2)
{
    ...
    // if segments not parallel, compute closest point on L1 to L2 and clamp segment S1. Else pick
    // arbitrary s (here 0)
    if (denom != 0.0f) {
        s = clamp((b * f - c * e) / denom, 0.0f, 1.0f);
    } else
        s = 0.0f;
    // Compute point on L2 closest to S1(s) using t = Dot((P1 + D1*s) - P2,D2) / Dot((D2,D2) = (b*s
    // + f) / e
    t = (b * s + f) / e;
